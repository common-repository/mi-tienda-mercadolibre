...
Text Domain: mi-tienda-mercadolibre
...
function mtmlplugin_init() {
	load_plugin_textdomain('mi-tienda-mercadolibre','',plugin_basename( dirname( __FILE__ ) .'/languages' ));
}
...
function do_tienda_ml( $atts ){
	...
	foreach ($data['results'] as $item) {
		...
		$output.=  '<a rel="nofollow" href="'.$item['permalink'].'" title="'.$item['title'].'" rel="nofollow" class="img-shadow"><img src="'.str_replace('s_MLA_v_I_', 's_MLA_v_O_', $item['thumbnail']).'" alt="'.$item['title'].': '.$item['subtitle'].'"/></a>';
		$output.=  '<h3>'.$item['title'].'<span class="mtml-precio">$'.$item['price'].'</span></h3>';
		if ($item['subtitle']) { $output.=  '<p>'.$item['subtitle'].'</p>'; };
		$output.=  '<a href="'.$item['permalink'].'" class="boton">'.__('Ver detalles','mi-tienda-mercadolibre').'</a>';
		$output.=  '</li>';
	}
	$output.= '</ul>';
